...
# all_peptideIons <- import_openswath_matrix_fromEulerPortal(search_results="E1702271600_matrix_requant",
...
In targeted mass spectrometry, it is rational and important to generate a quantitative protein matrix that contains a least number of missing values. Thus, we provide `imputate_missing_values()` to impute values following a uniform distribution between minimum $0.1 \times CV$ and minimum $1.1 \times CV$
...
These features can be plotted separately by `plot_bar_intensity_prob()`, `plot_cor_heatmap()` and `plot_peptide_intensity()`.
...
```{r plot_coefficients_model, fig.show='hold', fig.align='center', fig.width=5, fig.height=4}
