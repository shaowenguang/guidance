...
The feature-level statistics can be visualized using `ggplot2` into barplots, correlation matrix, trend lines and etc. In addiiton, we provide `plot_density()`, `plot_heatmap()`, `plot_bar_intensity_n_probability()` and `plot_protein_profile()` to visualize quantitative pattern of a feature (refer to Part 5: [Visualization for details](#visualization)). Here is an example demonstration of visualzing density distribution of average intensity using `plot_density()`. 
...
+ `perform_anova()` calculates differential expression pattern between and within groups of the same sample.  [more description based on how the function is built]
...
```{r statistical_tests, fig.show='hold', echo= FALSE, eval = FALSE}
# t-test 
