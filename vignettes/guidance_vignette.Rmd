...
(wenguang: sample_annotation is NULL? we need to check...)
```
...
(Wenguang: Chloe, I think the basic assumption for the usage of TIC is to assume that the same amount of proteins was injected for each MS run. That's why we can sum all the peptides identified to a certain number.)
...
(Wengunag: I think we may need to have more description for the function, merge_replicate... Actually, this function works with both "peptides" (as we used here) and "proteins" (as we will use after protein abundance estimation).)
...
(Wenguang: something may be wrong above?)
```
...
The feature-level statistics can be visualized using `ggplot2` into barplots, correlation matrix, trend lines and etc. In addiiton, we provide `plot_density()`, `plot_heatmap()`, `plot_bar_intensity_prob()` and `plot_a_heatmap_include_prob_update()` to visualize quantitative pattern of a feature (refer to Part 5: Visualization for details). Here is an example demonstration of visualzing density distribution of average intensity using `plot_density()`. 
...
For more details about training user-specific models if necessary, please see Part 4). 
...
(Wenguang: shall we descibe the histogram, e.g. the binomial dist. that indicates the sucess of the seperation of two groups of feature)
```
...
### selecting 
(Wenguang: this is the core of the algorithm. so i think it is good to keep it as an indepedent section)

To estimate robust protein abundances, the user can use the posterior probability predicted, to perform an optimal selection (0.2 by default) of peptide features that accurately and reliably represent their respective protein concentrations.

+ Filter peptides by posterior probability: users can increase this threshold to increase their confidence for peptide selection, e.g., probability > 0.4.
```{r protein_infer_filter, fig.show='hold'}
test_yesFiltered <- test[prob > 0.2, ]
```
(Wenguang: this is not urgent. but as discussed, maybe we can have an algorithm to automatically detect this threshold. It is not very diffcult to implement, which requires some knowledge of parametric fitting. I also expect this currently hard-coded threshold would be sth that the reviewers may attack...)

### imputation on missing values
+ Impute missing values: in targeted mass spectrometry, it is rational and important to generate a quantitative protein matrix that contains a least number of missing values. Thus, we provide `imputate_missing_values()` to impute values following a uniform distribution between minimum $0.1 \times CV$ and minimum $1.1 \times CV$
...
(Wenguang: chloe, bool is the indicator for the variable, which means it is a binary; not the model is boolean...)
...
# Train linear discriminant analysis (LDA) models 
In this part, we demonstrate how to train the LDA model that is used as a default model in the package. This model was trained by the QGS dataset. In addition, the user can, if necessary, also train their own model by using these commands. 
...
```{r plot_coefficients_model, fig.show='hold', fig.align='center', fig.width=5, fig.height=5}
