...
## Loading the data 
The package provides several options to import data table:

The package provides `import_openswath()` function to import SWATH data and sample annotation:
```{r load_data, fig.show='hold'}
library(Prom)
peptideIons <- import_openswath(search_results= "E:/DIA-guidance/feature_alignment.csv", 
                                sample_annotation="E:/DIA-guidance/sample_annotation", 
                                level="PeptideIon") 
```
...
## Prepare data
The SWATH data is in long format i.e. each row is one variable per peptide. But to facilitate downstream analysis to normalize, compute features and infer protein abundances, we provide a helpful utility function `long2wide()` to convert from long to wide format data, i.e. one peptide's variables in a single row.  
