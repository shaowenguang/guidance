...
To estimate robust protein abundances, the user can use the posterior probability predicted (0.2 by default), to perform an optimal selection of peptide features that accurately and reliably represent their respective protein concentrations.
...
Once the data is ready to proceed with protein inference step, we provide `pept2prot()` to summerize protein abundance from peptide/fragment-level intensities. We provide a number of parameters that users can denote for an optimal estimation of protein abundance. 
